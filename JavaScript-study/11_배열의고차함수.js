...
let stdList = [
    {id: 1, name: "박성규", java: 75, db: 80},
    {id: 2, name: "김성규", java: 60, db: 70}, 
    {id: 3, name: "이성규", java: 90, db: 95},
]
...
// filter : 조건을 통과한 요소만 남긴 새로운 배열 만듬 
// 각 요소를 전달해서 리턴값이 true인 요소만 남긴 새로운 배열 만듬
...
const found = stdList.find(std => std.name === "김성규");
console.log(found);

// findIndex : 조건을 만족하는 첫 요소의 인덱스 반환(없으면 -1)
const idx = stdList.findIndex(std => std.name === "김성규");
console.log(idx);

// some, every
const arr = [1,2,3,4,5];
console.log(arr.some(n => n > 3)); // 조건을 만족시키는 값이 한개라도 있으면 true
console.log(arr.every(n => n > 3)); // 전부 조건을 만족시키면 true 
